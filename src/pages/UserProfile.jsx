...
function ProfileAvatar({ name, era }) {
  ...
  return (
    <div
      className={`w-24 h-24 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center shadow-xl border-4 border-white dark:border-slate-700`}
    >
...
function InfoChips({ items }) {
  if (!items || items.length === 0) {
    return <p className="text-slate-400 dark:text-slate-500 italic text-sm">Not set</p>;
  }
  return (
...
export default function UserProfile() {
  ...
  const populateForm = (p) => {
    setFormData({
      ...
      life_experiences:
        p.life_experiences?.map((exp) => `${exp.title}: ${exp.description}`).join('\n') || '',
      favorite_music: p.favorite_music?.join(', ') || '',
      important_people:
        p.important_people?.map((per) => `${per.name} (${per.relationship})`).join(', ') || '',
          ...
          <div className="flex-1 text-white">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              {profile?.favorite_era && (
                <span className="bg-white/20 text-white text-sm px-3 py-1 rounded-full">
                  {ERA_LABELS[profile.favorite_era] || profile.favorite_era}
                </span>
              )}
        ...
        {!isEditing ? (
            ...
            {!profile && (
              <Card>
                <CardContent className="py-10 text-center text-slate-500">
                  ...
                  <p className="text-lg font-medium mb-4">No profile created yet</p>
          ...
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select
                    value={formData.favorite_era}
                    onValueChange={(v) => setFormData({ ...formData, favorite_era: v })}
                  >
                    <SelectTrigger className="min-h-[44px]">
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div>
                  ...
                  <Select
                    value={formData.communication_style}
                    onValueChange={(v) => setFormData({ ...formData, communication_style: v })}
                  >
                    <SelectTrigger className="min-h-[44px]">
                      <SelectValue />
                    </SelectTrigger>
            ...
            <Card>
              ...
              <CardContent>
                <Input
                  placeholder="e.g., Frank Sinatra, Moon River (comma separated)"
                  className="min-h-[44px]"
            ...
            <Card>
              ...
              <CardContent>
                <Input
                  placeholder="e.g., John (husband), Sarah (daughter)"
                  className="min-h-[44px]"

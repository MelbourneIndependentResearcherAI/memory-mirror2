...
export default function ConversationAnalyticsPage() {
  ...
  const conversationTrendData = useMemo(() => {
    return Array.from({ length: 7 }, (_, i) => {
      ...
      const dayLogs = chatLogs.filter(l => {
        ...
        return format(parseISO(l.created_date), 'yyyy-MM-dd') === dayKey;
      });
      return { day: dayStr, messages: dayLogs.length };
    });
  }, [chatLogs]);

  // Era distribution from chat logs
  ...
  // Sentiment by time of day from anxiety logs
  const sentimentMetrics = useMemo(() => {
    ...
    return Object.entries(buckets).map(([time, levels]) => {
      const avg = levels.length ? levels.reduce((a, b) => a + b, 0) / levels.length : 5;
      const positive = Math.round(Math.max(0, 100 - avg * 8));
      const negative = Math.round(Math.min(100, avg * 5));
      return { time, positive, neutral: Math.max(0, 100 - positive - negative), negative };
    });
  }, [anxietyLogs]);

  const totalChats = chatLogs.length;
  const avgAnxiety = anxietyLogs.length
    ? (anxietyLogs.reduce((s, l) => s + (l.anxiety_level ?? 5), 0) / anxietyLogs.length).toFixed(1)
    : 'N/A';
  const avgSentiment = anxietyLogs.length
    ? (10 - parseFloat(avgAnxiety)).toFixed(1) + '/10'
    : 'N/A';
  const thisWeekCount = chatLogs.filter(l => {
    if (!l.created_date) return false;
    return parseISO(l.created_date) >= subDays(new Date(), 7);
  }).length;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-12 text-center">
            ...
            <p className="text-slate-400 text-xl">No activity logged yet.</p>
            <p className="text-slate-500 mt-2">Conversations and events will appear here once your loved one begins using Memory Mirror.</p>
          </div>
        ) : (
          <>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-white">{avgSentiment}</p>
                  </div>
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-white">{activityLogs.length}</p>
                  </div>
